...
def plot_mesh(mesh, mesh_pose=np.eye(4)):
    """
    ...
    mesh_cam = mesh.vertices # homog_mesh_vert.dot(mesh_pose.T).dot(cam_trafo.T)[:,:3]
...
def show_image(rgb, segmap):
    ...
    if segmap is not None:
        ...
        plt.title('grasp_viz.py:show_image:46')
        plt.imshow(segmap, cmap=cmap, alpha=0.5, vmin=0.0001)
        
    print("Save figure to Contact_graspnet_figure.png")
    plt.savefig('Contact_graspnet_figure.png')

def visualize_grasps(full_pc, pred_grasps_cam, scores, hand_pcs=None, hand_cols=None, 
                     plot_opencv_cam=False, pc_colors=None, gripper_openings=None, gripper_width=0.06):
    ...
    Arguments:
        ...
        pred_grasps_cam {dict[int:np.ndarray]} -- Predicted 4x4 grasp tranforms per segment or for whole point cloud
        scores {dict[int:np.ndarray]} -- Confidence scores for grasps

        hand_pcs {np.ndarray} -- Hand pointcloud XYZ in Nx3
        hand_cols {np.ndarray} -- Hand pointcloud colors in Nx3
    ...
    # full_pc = full_pc.item()

    # pred_grasps_cam = list(pred_grasps_cam.item().values())
    # pred_grasps_cam = np.concatenate(pred_grasps_cam, axis=0)

    # print('pred_grasps_cam', type(pred_grasps_cam))
    ...
    if hand_pcs is not None and hand_cols is not None:
        # print('    Visualizing hands\n', hand_pcs)
        draw_pc_with_colors(np.concatenate(hand_pcs, axis=0), 
                            np.concatenate(hand_cols, axis=0), 
                            points_size=0.002)

    # colors = [cm(1. * i/len(pred_grasps_cam))[:3] for i in range(len(pred_grasps_cam))]
    # colors2 = {k:cm(0.5*np.max(scores[k]))[:3] for k in pred_grasps_cam if np.any(pred_grasps_cam[k])}
    ...
    for i,k in enumerate(pred_grasps_cam):
        if np.any(pred_grasps_cam[k]):
            store[np.max(scores[k])] = pred_grasps_cam[k][np.argmax(scores[k])] 
    ...
    for i,(s, v) in enumerate(store2.items()):
        # print(i, '/', len(store2))
        gripper_openings_k = np.ones(1) * gripper_width if gripper_openings is None else gripper_openings[k]
        colors3 = [cm2(0.5*s)[:3]]
        draw_grasps([v], np.eye(4), colors=colors3, gripper_openings=gripper_openings_k)
...
def draw_pc_with_colors(pc, pc_colors=None, points_size=0.0018, single_color=(0.3,0.3,0.3), mode='point', scale_factor=0.0018):
    """
    ...
    Arguments:
        ...
        points_size -- size of points rendered
...
#     fig = plt.figure('Grasp Candidates')
...
#     # Get all grasps with score above 80th percentile
#     med = np.percentile(list(store.keys()), 80)
#     store2 = {s: v for s,v in store.items() if s > med}
...
#     # mlab.show()
#     # mlab.view() # azimuth=180, elevation=180, distance=1)
#     # mlab.view(azimuth=120, elevation=180, distance=0.5, focalpoint=(0, 0, 0))
#     plt.savefig(fname='robot_grasp_viz.png', dpi=(300))
#     print('Save robot_grasp_viz.png')
#     plt.show(block=True)
...
#     scalars = pc_colors[:,0]*256**2 + pc_colors[:,1]*256 + pc_colors[:,2]
#     rgb_lut = create_8bit_rgb_lut()
#     points_mlab = plt.scatter(pc[:, 0], pc[:, 1], pc[:, 2], c=scalars)
...
def draw_grasps(grasps, cam_pose, gripper_openings, color=(0,1.,0), colors=None, show_gripper_mesh=False, tube_radius=0.0008):
    ...
    gripper_control_points = get_control_point_tensor(1, False).squeeze()
...
#     plt.plot(all_pts[:,0], all_pts[:,1], all_pts[:,2])
#     # src.mlab_source.dataset.lines = connections
#     # src.update()
#     # lines =mlab.pipeline.tube(src, tube_radius=tube_radius, tube_sides=12)
#     # mlab.pipeline.surface(lines, color=color, opacity=1.0)
